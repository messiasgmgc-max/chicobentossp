...
export const PlaceCard: React.FC<PlaceCardProps> = ({
  ...
  const getStatusBadge = (status: PlaceStatus) => {
    switch (status) {
      case 'planned':
        return (
          <span className="px-2 py-0.5 sm:px-2.5 sm:py-1 rounded-full text-[10px] sm:text-[11px] font-semibold bg-blue-500/20 text-blue-300 border border-blue-500/30 flex items-center gap-1 shrink-0">
      ...
      <div className="p-3.5 sm:p-5 flex-1 flex flex-col justify-between space-y-3">
        <div>
          <h3
            ...
            className="text-sm sm:text-base font-bold text-white hover:text-blue-400 cursor-pointer transition-colors line-clamp-1"
          >
            {place.name}
          </h3>
          ...
          <div className="flex items-center gap-1 shrink-0">
            ...
            {onQuickAddToDay && (
              <button
                ...
                className="px-2.5 sm:px-3 py-1.5 bg-blue-600 hover:bg-blue-500 text-white rounded-xl text-xs font-semibold transition-transform active:scale-95 flex items-center gap-1 shadow-md shadow-blue-600/20"
