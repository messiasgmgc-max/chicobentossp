...
interface PlacesCatalogProps {
  ...
  onOpenAddPlace: (initialQuery?: string) => void;
      ...
      {sortedPlaces.length === 0 ? (
        <div className="bg-slate-900/60 border border-dashed border-slate-800 rounded-3xl p-8 sm:p-12 text-center">
          ...
          <h3 className="text-base font-bold text-white mb-1">
            {searchTerm ? `Nenhum local salvo como "${searchTerm}"` : 'Nenhum local encontrado'}
          </h3>
          <p className="text-xs text-slate-400 mb-4 max-w-sm mx-auto">
            {searchTerm
              ? `Você pode pesquisar "${searchTerm}" diretamente no Google Maps para importar nome, endereço, metrô e coordenadas GPS automaticamente!`
              : 'Ajuste os filtros ou adicione um novo local para o grupo.'}
          </p>
          <button
            onClick={() => onOpenAddPlace(searchTerm.trim() || undefined)}
            className="px-4 py-2.5 bg-blue-600 hover:bg-blue-500 text-white rounded-xl text-xs font-semibold inline-flex items-center gap-2 active:scale-95 shadow-md shadow-blue-600/20"
          >
            <Plus className="w-4 h-4" />
            <span>{searchTerm ? `Buscar "${searchTerm}" no Google Maps` : 'Cadastrar Novo Local'}</span>
          </button>
        </div>
      ) : (
